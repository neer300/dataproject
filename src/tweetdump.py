...
def get_geocode_for_cities(cities):
    ...
    for city in cities:
        ...
        cities_long_names.append(place)
    print cities_geo
    return (cities_geo,cities_long_names)

api = tweepy.API(retry_delay=15)
...
terms = 'pot OR weed OR grass OR marijuana'
summary_term = "marijuana"

cities_name = ["New York, NY, USA","Seattle, WA, USA", "San Francisco, CA, USA", "Austin, TX, USA", "Los Angeles","Madison, WI, USA","Chicago","Miami","Atlanta","Detroit"]
cities_name +=  ["Portland","Las Vegas","Salt Lake City","Denver","Houston","Phoenix","Philadelphia","San Diego","San Antonio","Detroit","San Jose","Indianapolis","Jacksonville","Columbus","Memphis","Baltimore"]
cities_name += ["Milwaukee","Boston","Charlotte","El Paso","Fort Worth","Nashville-Davidson","Oklahoma City"]

ret = get_geocode_for_cities(cities_name)
cities = ret[0]
cities_name = ret[1]


for i in range(len(cities)):
    fout = open("../resources/OldTweets_"+summary_term+"_"+cities_name[i]+".txt",'w')
